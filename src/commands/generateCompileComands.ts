        ...
        (async () => {
            ...
            if (os === "win32") {
                ...
                shellCommand = new vscode.ShellExecution(
                    `"${unrealBuildToolPath}" -project=${path.join(projectFolder, project.Modules[0].Name)}.uproject ${project.Modules[0].Name}Editor ${buildOsType} Development`,
                    { cwd: unrealEngineInstallation, executable: runtimePath}
                );
            } else if (os === "darwin") {
                buildOsType = "Mac";
                shellCommand = new vscode.ShellExecution(
                    `${runtimePath.split(" ").join("\\ ")} ${unrealBuildToolPath.split(" ").join("\\ ")} -project=${path.join(projectFolder, project.Modules[0].Name).split(" ").join("\\ ")}.uproject ${project.Modules[0].Name}Editor ${buildOsType} Development`,
